...
STAKE_CONTRACT = Web3.to_checksum_address("0x3691A78bE270dB1f3b1a86177A8f23F89A8Cef24")
STAKE_ABI = [
    {"name": "stake", "type": "function", "stateMutability": "payable", "inputs": [], "outputs": []}
]

MIN_DACC_BALANCE = 1.0
STAKE_PCT_MIN = 0.10
STAKE_PCT_MAX = 0.25
...
def confirm_stake(
    ...
    for attempt in range(1, CONFIRM_RETRIES + 1):
        try:
            resp = session.post(
                f"{API_BASE}/inception/exchange/confirm-stake/",
            ...
            try:
                ...
                pass
            logger.warning(
                f"{account.profile_number} ⚠️ confirm-stake, попытка {attempt}/{CONFIRM_RETRIES}: {e}{body}"
            )
            if attempt < CONFIRM_RETRIES:
                time.sleep(random.uniform(*CONFIRM_RETRY_PAUSE))

    logger.error(
        f"{account.profile_number} confirm-stake не удался после {CONFIRM_RETRIES} попыток"
    )
    return None
...
def accounts_filter(accounts: list[Account]) -> list[Account]:
    ...
    for account in accounts:
        status = get_value_from_txt(account, "stake_status.txt")
        if status == "SUCCESS":
            logger.warning(f"{account.profile_number} ⚠️ Пропуск — stake уже выполнен")
            continue
...
def worker(account: Account) -> None:
    ...
    pct = random.uniform(STAKE_PCT_MIN, STAKE_PCT_MAX)
    decimals = random.randint(1, 4)
    stake_ether = round(balance.ether * pct, decimals)
    stake_amount = Amount(round(stake_ether * 10**18), wei=True)

    logger.info(
        f"{account.profile_number} Баланс: {balance.ether:.{decimals}f} DACC | "
        f"Стейкаем: {stake_ether:.{decimals}f} DACC ({pct * 100:.1f}%)"
    )

    contract = onchain.w3.eth.contract(address=STAKE_CONTRACT, abi=STAKE_ABI)

    tx_hash = None
    for attempt in range(1, STAKE_TX_RETRIES + 1):
        try:
            tx_params = onchain._prepare_tx(value=stake_amount)
            ...
            logger.warning(
                f"{account.profile_number} ⚠️ TX, попытка {attempt}/{STAKE_TX_RETRIES}: {e}"
            )
            if attempt < STAKE_TX_RETRIES:
                time.sleep(random.uniform(*STAKE_TX_RETRY_PAUSE))

    if not tx_hash:
        logger.error(f"{account.profile_number} TX не прошёл после {STAKE_TX_RETRIES} попыток")
        return
    ...
    if not session:
        logger.error(
            f"{account.profile_number} Реавторизация перед confirm-stake не удалась | TX: {tx_hash}"
        )
        bot = SimpleBot(account)
        cell_value_to_txt(bot, "SUCCESS", "stake_status.txt")
        return

    result = confirm_stake(session, account, csrf_token, tx_hash)
    ...
    if result:
        logger.success(
            f"{account.profile_number} 🔥 Stake выполнен | "
            f"Застейкано: {stake_ether:.{decimals}f} DACC | TX: {tx_hash[:16]}..."
        )
    else:
        logger.warning(
            f"{account.profile_number} ⚠️ TX прошёл, confirm-stake не ответил | TX: {tx_hash}"
        )
...
def main():
    ...
    for cycle in range(cycles):
        ...
        if not active:
            logger.warning("⚠️ Все аккаунты уже застейкали DACC")
            if cycle < cycles - 1:
