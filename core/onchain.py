...
class Onchain:
    ...
    def _prepare_w3(self, chain: Chain) -> Web3:
        # Один UA на весь запуск аккаунта: берём из account.user_agent если задан,
        # иначе выбираем случайный (для обратной совместимости)
        ua = getattr(self.account, 'user_agent', None) or get_user_agent()
        request_kwargs = {
            'headers': {
                'User-Agent': ua,
    ...
    def _sign_and_send(self, tx: dict) -> str:
        ...
        if tx_receipt['status'] != 1:
            raise ValueError(
                f'Транзакция reverted (status=0): {tx_receipt["transactionHash"].hex()}'
            )
    ...
    def _get_approval_logs(self):
        ...
        ua = getattr(self.account, 'user_agent', None) or get_user_agent()
        response = get_response(url, params, proxy=self.account.proxy, user_agent=ua)
